...
可用工具：
- search_docs: 在文档中搜索相关内容（支持 GraphRAG 增强搜索）
  - 默认使用"混合搜索"模式，结合向量相似度搜索和图遍历搜索
  - 图遍历搜索通过文档之间的关系（如相同来源、相同文件、相同标题）找到更多相关内容
  - 可选搜索模式：vector（纯向量搜索）、graph（图遍历搜索）、hybrid（混合搜索，默认）
- list_docs: 列出所有可用文档
- read_doc: 读取指定文档的完整内容（当知道文档名时直接使用）
- refresh_docs: 刷新文档索引（注意：首次使用 GraphRAG 时建议刷新索引）
"""
