...
aliases: [TreeV2]
version: 1.0.0
description: "PacvueTreeV2 树形组件的一般使用"
---

# TreeV2 组件文档

> PacvueTreeV2 树形组件的一般使用

**分类**: data | **标签**: 虚拟树、大数据树、V2
...
<template>
  <PacvueTreeV2
    ...
    :treeGroupOpen="!false"
    tooltip
    :pausedTooltipAlone="!true"
    :onlyOneSetCheck="!true"
    :data="tdata"
    :option="option"
    :clearable="true"
    ref="treeRef"
...
| sort | 示例函数 getCheckedList 返回值是否按照顺序排序 | Boolean | false |
...
| load | 异步加载子列表数据,需要在单列数据中配置hasChildren为true | Function(item,resolve) | -- |
| expandIconStyle | 折叠的图标样式,需要返回样式对象 | Function(item) | -- |
| isFreezeByChildren | 冻结节点是否根据子的推导出来 | Boolean | true |
| dataCy | node节点的自动化测试埋点属性dataCy | String | -- |
| dataCyFilter | tree组件的过滤功能的自动化测试埋点属性dataCy | String | -- |
...
| 事件名 | 说明 | 回调参数 |
| --- | --- | --- |
| nodeChange | 单个节点点击事件 | id, checkType |
...
| itemRender | v-slot:itemRender="node"：单个节点样式插槽，不设置则展示默认 label | -- |
