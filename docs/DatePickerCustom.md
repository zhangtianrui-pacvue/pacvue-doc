...
category: form
tags: [自定义日期, 表单]
aliases: [DatePickerCustom]
version: 1.0.0
description: "PacvueDatePickerCustom 定制日期选择"
---

# DatePickerCustom 组件文档

> PacvueDatePickerCustom 定制日期选择
...
| minDate | 配置可选最小日期 | Date | null |
| maxDate | 配置可选最大日期 | Date | null |
...
| dateFormatText | 显示时间格式化字符串，优先级高于本地缓存中根据平台差异判断的日期格式。 | String | MM/DD/YYYY |
...
| showCompareCustom | 是否在compare中显示Custom模式 | Boolean | true |
...
| compareDateFormat | compare 数据的格式化方式 | String | MM/DD/YYYY |
| showQuickOperation | 是否启用快速切换周期功能 | Boolean | true |
...
| 方法名 | 说明 | 参数 |
| --- | --- | --- |
| resetCompareDate | 刷新当前 compare date | () => {} |
| selectDateRange | 选中某个右侧 date range | (quickMode) => {} |
...
| startSelectDate | 自定义框选选中第一个日期时触发事件，compare 自定义框选时也会触发。 | (event: Date, isCompare: Boolean) => {} |
| completeSelectDate | 自定义框选选中第二个日期时触发事件，compare 自定义框选时也会触发。 | (event: Date, isCompare: Boolean) => {} |
